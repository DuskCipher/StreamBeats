...
String convertJsonToM3U(Map<String, dynamic> jsonData) {
  try {
    ...
    final generatedBy = meta['generated_by']?.toString() ?? "StreamBeats";
    ...
    buffer.writeln("#STREAMBEATS-GENERATED_BY: $generatedBy");
    buffer.writeln("#STREAMBEATS-VERSION: $version");
    buffer.writeln("#STREAMBEATS-EXPORTEDAT: $exportedAt");
    buffer.writeln("#STREAMBEATS-NOTE: $note");
    buffer.writeln();
...
Map<String, dynamic> parseM3UToJson(String m3uContent) {
  try {
    ...
    for (final line in lines) {
      ...
      if (line.startsWith('#STREAMBEATS-GENERATED_BY:') || line.startsWith('#BLOOMEE-GENERATED_BY:')) {
        meta['generated_by'] = line.split(':').last.trim();
      } else if (line.startsWith('#STREAMBEATS-VERSION:') || line.startsWith('#BLOOMEE-VERSION:')) {
        meta['version'] = line.split(':').last.trim();
      } else if (line.startsWith('#STREAMBEATS-EXPORTEDAT:') || line.startsWith('#BLOOMEE-EXPORTEDAT:')) {
        meta['exportedAt'] = line.split(':').last.trim();
      } else if (line.startsWith('#STREAMBEATS-NOTE:') || line.startsWith('#BLOOMEE-NOTE:')) {
