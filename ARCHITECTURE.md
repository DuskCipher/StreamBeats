...
|---|---|---|
| `StreamBeatsPlayerCubit` | `blocs/media_player/` | Wraps `StreamBeatsMusicPlayer`; exposes player state |
...
Plugins are WebAssembly modules conforming to WIT-defined interfaces, compiled via the `StreamBeats_plugin_sdk`. The Rust layer (`rust/`) bridges Flutter and the WASM runtime.
...
| Service | Purpose |
|---|---|
| `StreamBeatsMusicPlayer` | Core audio engine wrapping `audio_service` |
...
App start
  └─ GlobalEventsCubit.checkForUpdates()
       └─ getAppUpdates()          (services/StreamBeats_updater_tools.dart)
            ├─ GitHub Releases API
            ...
            Result map keys:
              ...
              changelogs → String?: text if unread changelog exists
                  ...
                  └─ results == true  &&  autoUpdateNotify setting == true
                        ...
                        → GlobalEventListener: showStreamBeatsDialog with localised strings
                             ├─ "Later" → dismiss
